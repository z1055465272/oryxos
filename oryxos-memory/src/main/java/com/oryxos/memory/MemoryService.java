...
/**
 * Memory 三层记忆统一门面。对 ReAct 循环只暴露一个接口， 内部把会话记忆委托给 SessionManager、长期记忆委托给 LongTermMemory。
 *
 * <p>核心阶段做会话 + 长期两层（MEMORY.md 文件 + save_memory / recall_memory 两个内置 Tool）， 情景记忆和向量检索放扩展阶段。{@code
 * recallMemory} 预留向量检索升级空间。
 */
